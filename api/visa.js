      ...
      for (const trav of travelers) {
        ...
        for (const nat of nationalities) {
          for (const resStatus of residencies) {
            ...
            try {
              if (db) {
                ...
                if (snapshot.exists()) {
                  ...
                  if (Date.now() - cached.updatedAt.toMillis() < cacheTTL) { cachedResult = cached.data; break; }
                }
              }
            ...
            if (!key) continue;
            try {
              const url = `https://travel-buddy.p.rapidapi.com/api/v1/visa-policy?nationality=${encodeURIComponent(nat)}&destination=${encodeURIComponent(destName)}`;
        ...
        out[destName].push({ nationality: nationalities[0], residencies, visaRequired: cachedResult?.visaRequired || visaResult.visaRequired, visaFee: (cachedResult && typeof cachedResult.visaFee === 'number') ? cachedResult.visaFee : visaResult.visaFee });
      }
    }
    res.json(out);
