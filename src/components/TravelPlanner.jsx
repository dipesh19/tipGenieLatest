...
export default function TravelPlanner(){
  useEffect(()=>{
    ...
    try{ const conf = process.env.NEXT_PUBLIC_FIREBASE_CONFIG; if(conf) initFirebase(JSON.parse(conf)); }catch(e){}
  },[]);
  ...
  const destinationOptions = [{value:'Rome',label:'Rome'},{value:'Paris',label:'Paris'},{value:'Masai Mara',label:'Masai Mara'},{value:'Tokyo',label:'Tokyo'},{value:'Bali',label:'Bali'}];
  ...
  const handleDestinationsChange = (selected)=>{ const limited = (selected||[]).slice(0,5); setFormData(p=>({...p, selectedDestinations: limited })); };

  async function fetchIntegratedResults(){
    setLoading(true);
    try{
      const chosen = formData.selectedDestinations.length? formData.selectedDestinations.map(d=>d.value) : destinationOptions.map(d=>d.value);
      const imgRes = await fetch('/api/images',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({queries:chosen})});
      const imgJson = imgRes.ok?await imgRes.json():{destinations:chosen.map(q=>({name:q,image:'/placeholder.png'}))};
      const destNames = imgJson.destinations.map(d=>d.name);
      const flightsRes = await fetch('/api/flights',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({origin:formData.travelers[0]?.nationalities?.[0]||'JFK',destinations:destNames,dates:{start:formData.startDate,end:formData.endDate}})});
      const flightsJson = flightsRes.ok?await flightsRes.json():{};
      const visaPayload = { travelers: formData.travelers.map(t=>({ name: t.name, nationalities: t.nationalities||[], residencies: t.residencies||[] })), destinations: destNames };
      const visaRes = await fetch('/api/visa',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(visaPayload)});
      const visaJson = visaRes.ok?await visaRes.json():{};
      const costsRes = await fetch('/api/costs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({destinations:destNames})});
      const costsJson = costsRes.ok?await costsRes.json():{};

      const combined = destNames.map((dest, idx)=>{
        const img = imgJson.destinations[idx]?.image || '/placeholder.png';
        const flight = flightsJson[dest] || { price: Math.floor(Math.random()*400+300) };
        const costInfo = costsJson[dest] || { dailyCost: Math.floor(Math.random()*80+30), avgDailyExpense: Math.floor(Math.random()*120+50) };
        const visaForDest = visaJson[dest] || [];
        const travelerBreakdown = formData.travelers.map((t, tIdx)=>{
          const visaInfo = visaForDest[tIdx] || {};
          const visaFee = typeof visaInfo.visaFee === 'number' ? visaInfo.visaFee : (visaInfo.visaRequired === 'Required' ? estimateDefaultVisaFee(visaInfo, dest) : 0);
          const individualCost = calculateShared((flight.price||0) + (costInfo.dailyCost||0)*5 + visaFee, formData.travelers);
          return { name: t.name || `Traveler ${tIdx+1}`, nationalities: t.nationalities||[], residencies: t.residencies||[], visaNeeded: visaInfo.visaRequired || 'Unknown', visaFee, individualCost };
        });
        const totalVisa = travelerBreakdown.reduce((s,tb)=>s+(tb.visaFee||0),0);
        const totalEstimated = Math.round((flight.price||0) + (costInfo.avgDailyExpense||0)*5 + totalVisa);
        return { destination: dest, image: img, flightCost: flight.price||0, dailyCost: costInfo.dailyCost||0, avgDailyExpense: costInfo.avgDailyExpense||0, totalVisa, totalCost: totalEstimated, travelerBreakdown };
      });

      setResults(combined);
      try{ await logSearch({ query: formData, resultsCount: combined.length }); }catch(e){}

    }catch(err){ console.error(err); alert('Live API fetch failed; check console.'); }
    setLoading(false);
  }

  const handleSubmit = async (e)=>{ e?.preventDefault(); await fetchIntegratedResults(); };

  function estimateDefaultVisaFee(visaInfo, destination){ const destLower = (destination||'').toLowerCase(); if(destLower.includes('paris')||destLower.includes('rome')) return 90; if(destLower.includes('new york')||destLower.includes('usa')) return 160; if(destLower.includes('india')) return 25; if(destLower.includes('tokyo')||destLower.includes('japan')) return 30; return 80; }

  return (
    <div>
      <div style={{textAlign:'center',marginBottom:16}}>
        <h1 style={{fontSize:28}}>✈️ Trips Genie - Plan Smart, Travel Better</h1>
        <p style={{color:'#444'}}>Compare up to 5 destinations — multi-nationality & multi-residency supported.</p>
      </div>
      ...
      <Card>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{marginTop:12}}>
            <label className='block text-sm'>Select destinations to compare (up to 5)</label>
            <Select options={destinationOptions} isMulti onChange={handleDestinationsChange} value={formData.selectedDestinations} placeholder='Pick destinations...'/>
          </div>
